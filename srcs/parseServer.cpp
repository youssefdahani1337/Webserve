...
void	Server::_listen(std::string &value)
{
	...
	{
		std::cerr << "WebServer: invalid port in " << port;
...
void	Server::_hostDirective(std::string &value)
{
	...
	if (dot != '.' || !issValue.eof())
	{
		std::cout << hostByte << dot << std::endl;
		std::cerr << "Error2: host not found " << host ;
...
void	Server::_serverNamesDirective(std::string &value)
{
	...
	while (issValue >> serverName)
	{
		...
		while (it != this->_serverNames.end())
		{
			if (*it == serverName)
...
void	Server::_maxBodySizeDirective(std::string &value)
{
	...
	issValue >> maxSize >> typeByte >> std::ws;
	if (issValue.fail() || !issValue.eof() || (typeByte != 'M' && typeByte != 'G'))
